#!/usr/bin/env node
//
// Releases the datocms CLI, its legacy alias and the two import plugins.
//
...
const main = async () => {
  ...
  if (bumping) {
    ...
    // `release: v4.0.30` when the release lands everything on one version,
    // which is the usual outcome in a linked group, and the explicit list when
    // only part of the group moves and the versions stay apart.
